$(document).ready(function() {
    ...
    $('#membershipTable').DataTable({
        "ajax": {
            ...
            "dataSrc": function (json) {
                // Check if there are any records and set membership status if available
                if (json.length > 0) {
                    var membershipNumber = json[0].membershipNumber;
                    var membershipName = getMembershipName(membershipNumber);
                    $('#membershipStatus').text('You are a ' + membershipName + ' Member');
                } else {
                    $('#membershipStatus').text('Membership details not available');
                }
                return json; // Return the processed data
            }
        },
        "columns": [
    ...
    function getMembershipName(membershipNumber) {
    if (membershipNumber >= 1 && membershipNumber <= 100) {
        return "Full Access";
    } else if (membershipNumber > 100 && membershipNumber <= 200) {
        return "Limited Access";
    } else if (membershipNumber > 200 && membershipNumber <= 300) {
        return "Seasonal";
    } else {
        return "Unknown";
    }
